...
if __name__ == '__main__':
    ...
    try:
        root1 = bisection_method(f, 0, 1)
        print(bcolors.OKBLUE, f"\nThe equation f(x) has an approximate root at x = {root1}", bcolors.ENDC)
    except Exception as e:
        print("There is no root in the given interval", e)

    try:
        root2 = bisection_method(f, -3, root1)
        print(bcolors.OKBLUE, f"\nThe equation f(x) has an approximate root at x = {root2}", bcolors.ENDC)
    except Exception as e:
        print("There is no root in the second given interval", e)
